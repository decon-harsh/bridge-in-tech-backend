    ...
    def post(cls,organization_id,program_id):
        ...
        if not is_wrong_token:
            try:
                ...
                # Checking organization exists
                organization = OrganizationModel.query.filter_by(id=organization_id).first() 
                if not organization:
                    return f"{messages.ORGANIZATION_DOES_NOT_EXIST}", HTTPStatus.NOT_FOUND 
                
                # Checking program exists
                program = ProgramModel.find_by_id(program_id)
                if not program or (program.organization_id != organization_id):
                    return f"{messages.PROGRAM_DOES_NOT_EXIST}", HTTPStatus.NOT_FOUND
                ...
                if response.status_code == 201:
                    mentorshipRelationId = MentorshipRelationModel.query.filter_by(mentor_id=mentor_id).filter_by(mentee_id=user['id']).first().id
